...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (data) {
            /* Recorre los datos que encuentra, captura el id razas (un div en el HTML) y le agrega para cada ítem un boton con un evento que devuelve el valor del elemento clickeado*/
            data.message.forEach(function (raza) {
                $('#razas').append(`<button type="button" onclick="verImg('${raza}')" class="btn btn-danger text-white" style="margin-bottom: 20px" data-bs-toggle="modal" data-bs-target="#modalButton">
                        ${raza}
                      </button>`);
            })
        }
    ...
    /*La función verImg toma como parametro la raza*/
    verImg = (raza) => {
